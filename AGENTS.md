...
- `src/testing/`: explicit testing facade used by non-unit suites that need lower-level control than `Machine` alone exposes. Includes
  `scene_recon.zig`, the frame-scene reconstruction shared by the `dump-scene` tool and the scene-pipeline regression tests.
...
- `src/scene.zig`: portable frame scene description (tilemap, sprites, palette, tile atlas, and viewport) for frontends that render the picture as 3D
  geometry instead of pixels. Extraction is a read-only observation of VDP state and must never change emulation behavior.
- `src/sms/`: Sega Master System and Game Gear emulation: VDP Mode 4, memory bus with Sega mapper, I/O port dispatch, Z80-based machine, PSG-only
  audio, GG 12-bit CRAM, GG viewport cropping, and Mode 4 frame scene extraction (`sms/scene.zig`).
...
- `web/scene3d.js`: WebGL2 renderer that draws a frame scene as depth-separated 3D layers. Context-agnostic, so the desktop orbit view and the
  WebXR theater share it. Its `OFF` offset table is a binary contract with `src/scene.zig`; the layout test there pins the offsets, and
  `sandopolis_scene_layout_version()` guards against a mismatch at runtime.
- `web/profiles/`: per-game 3D profiles as JSON, named after a ROM content hash (Master System headers carry no title, so the hash is the only
  dependable key). A missing profile is normal and means uniform extrusion. The directory is gitignored because `zig build mine-profile` fills it
  with reproducible machine-generated drafts; hand-tuned profiles are deliberate content and are committed with `git add -f`.
- `web/`: browser frontend for the WebAssembly build (HTML, JavaScript, AudioWorklet, and WebXR theater scene). `web/fonts/`, `web/img/`, and
  `web/sandopolis.wasm` are generated by `make web` and gitignored.
...
- `make web-test` after any change to `web/sandopolis.js`, `web/scene3d.js`, or `web/vr.js`. It drives the real page in headless Chromium and fails
  on any uncaught page exception. Neither `node --check` nor the shader harnesses execute `sandopolis.js`, so a deleted function or a bad binding is
  invisible to them. Pass `ROM=<path to an .sms or .gg file>` to include the 3D checks; without a ROM they are skipped.
