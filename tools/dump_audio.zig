...
fn usageError() error{InvalidArgs} {
    std.debug.print(
        "Usage: zig build dump-audio -- <sandopolis|gpgx> <rom-path> <wav-path> [frames] [--skip frames] [--mode normal|ym-only|psg-only]\n",
        .{},
    );
...
fn dumpSandopolis(allocator: std.mem.Allocator, config: Config) !void {
    ...
    std.debug.print(
        "sandopolis: wrote {d} frames to {s} ({d:.2}s) | ym_writes={d} ym_dac={d} psg_cmds={d} overflows={d} z80_pc=0x{X:0>4} busack=0x{X:0>4} reset=0x{X:0>4} ym_key=0x{X:0>2} dac_en=0x{X:0>2}\n",
        .{
