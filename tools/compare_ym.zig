...
const triad_mix_writes = [_]ScheduledWrite{
    // Channel 1: left
    scheduled(0, 0, 0xA4, 0x22),
    scheduled(1, 0, 0xA0, 0x80),
    scheduled(2, 0, 0xB0, 0x07),
    scheduled(3, 0, 0xB4, 0x80),
    scheduled(4, 0, 0x30, 0x01),
    scheduled(5, 0, 0x40, 0x18),
    scheduled(6, 0, 0x50, 0x1F),
    scheduled(7, 0, 0x60, 0x0C),
    ...
    // Channel 2: right
    scheduled(32, 0, 0xA5, 0x22),
    scheduled(33, 0, 0xA1, 0xC0),
    scheduled(34, 0, 0xB1, 0x07),
    scheduled(35, 0, 0xB5, 0x40),
    scheduled(36, 0, 0x31, 0x01),
    scheduled(37, 0, 0x41, 0x18),
    scheduled(38, 0, 0x51, 0x1F),
    scheduled(39, 0, 0x61, 0x0C),
    scheduled(40, 0, 0x71, 0x08),
    scheduled(41, 0, 0x81, 0x24),
    scheduled(42, 0, 0x39, 0x01),
    scheduled(43, 0, 0x49, 0x18),
    scheduled(44, 0, 0x59, 0x1F),
    scheduled(45, 0, 0x69, 0x0C),
    scheduled(46, 0, 0x79, 0x08),
    scheduled(47, 0, 0x89, 0x24),
    scheduled(48, 0, 0x35, 0x01),
    scheduled(49, 0, 0x45, 0x18),
    scheduled(50, 0, 0x55, 0x1F),
    scheduled(51, 0, 0x65, 0x0C),
    scheduled(52, 0, 0x75, 0x08),
    scheduled(53, 0, 0x85, 0x24),
    scheduled(54, 0, 0x3D, 0x01),
    scheduled(55, 0, 0x4D, 0x00),
    scheduled(56, 0, 0x5D, 0x1F),
    scheduled(57, 0, 0x6D, 0x0C),
    scheduled(58, 0, 0x7D, 0x08),
    scheduled(59, 0, 0x8D, 0x24),
    scheduled(60, 0, 0x28, 0xF1),

    // Channel 3: center
    scheduled(64, 0, 0xA6, 0x23),
    scheduled(65, 0, 0xA2, 0x20),
    scheduled(66, 0, 0xB2, 0x07),
    scheduled(67, 0, 0xB6, 0xC0),
    scheduled(68, 0, 0x32, 0x01),
    scheduled(69, 0, 0x42, 0x18),
    scheduled(70, 0, 0x52, 0x1F),
    scheduled(71, 0, 0x62, 0x0C),
    scheduled(72, 0, 0x72, 0x08),
    scheduled(73, 0, 0x82, 0x24),
    scheduled(74, 0, 0x3A, 0x01),
    scheduled(75, 0, 0x4A, 0x18),
    scheduled(76, 0, 0x5A, 0x1F),
    scheduled(77, 0, 0x6A, 0x0C),
    scheduled(78, 0, 0x7A, 0x08),
    scheduled(79, 0, 0x8A, 0x24),
    scheduled(80, 0, 0x36, 0x01),
    scheduled(81, 0, 0x46, 0x18),
    scheduled(82, 0, 0x56, 0x1F),
    scheduled(83, 0, 0x66, 0x0C),
    scheduled(84, 0, 0x76, 0x08),
    scheduled(85, 0, 0x86, 0x24),
    scheduled(86, 0, 0x3E, 0x01),
    scheduled(87, 0, 0x4E, 0x00),
    scheduled(88, 0, 0x5E, 0x1F),
    scheduled(89, 0, 0x6E, 0x0C),
    scheduled(90, 0, 0x7E, 0x08),
    scheduled(91, 0, 0x8E, 0x24),
    scheduled(92, 0, 0x28, 0xF2),

    scheduled(1400, 0, 0x28, 0x00),
    scheduled(1401, 0, 0x28, 0x01),
    scheduled(1402, 0, 0x28, 0x02),
};
...
// CSM rapid retriggering: Timer A period=0x3FF (overflow every 1 tick).
const csm_rapid_writes = [_]ScheduledWrite{
    // Channel 3 setup (same operators as csm_tone)
    scheduled(0, 0, 0xA6, 0x22),
    scheduled(1, 0, 0xA2, 0x80),
    scheduled(2, 0, 0xB2, 0x07),
    scheduled(3, 0, 0xB6, 0xC0),
    ...
    scheduled(26, 0, 0x7E, 0x08),
    scheduled(27, 0, 0x8E, 0x24),
    // Timer A = 0x3FF (fastest overflow)
    scheduled(28, 0, 0x24, 0xFF),
    scheduled(29, 0, 0x25, 0x03),
    // CSM enable + Timer A load + Timer A enable
    scheduled(30, 0, 0x27, 0x85),
    scheduled(3072, 0, 0x27, 0x84),
};

// CSM with operator parameter change mid-retriggering.
const csm_param_change_writes = [_]ScheduledWrite{
    scheduled(0, 0, 0xA6, 0x22),
    scheduled(1, 0, 0xA2, 0x80),
    scheduled(2, 0, 0xB2, 0x07),
    scheduled(3, 0, 0xB6, 0xC0),
    ...
    scheduled(29, 0, 0x25, 0x03),
    scheduled(30, 0, 0x27, 0x85),
    // Change op1 TL at clock 512
    scheduled(512, 0, 0x42, 0x30),
    // Change op1 TL again at clock 768
    scheduled(768, 0, 0x42, 0x08),
    scheduled(896, 0, 0x27, 0x84),
};

// CSM with all 8 FM algorithms cycled during retriggering.
const csm_all_algos_writes = [_]ScheduledWrite{
    scheduled(0, 0, 0xA6, 0x22),
    scheduled(1, 0, 0xA2, 0x80),
    scheduled(2, 0, 0xB2, 0x00), // start with algo 0
    scheduled(3, 0, 0xB6, 0xC0),
    scheduled(4, 0, 0x32, 0x01),
    scheduled(5, 0, 0x42, 0x18),
    scheduled(6, 0, 0x52, 0x1F),
    ...
    scheduled(29, 0, 0x25, 0x03),
    scheduled(30, 0, 0x27, 0x85),
    // Cycle through all 8 algorithms at 512-clock intervals
    scheduled(512, 0, 0xB2, 0x01),
    scheduled(1024, 0, 0xB2, 0x02),
    scheduled(1536, 0, 0xB2, 0x03),
    scheduled(2048, 0, 0xB2, 0x04),
    scheduled(2560, 0, 0xB2, 0x05),
    scheduled(3072, 0, 0xB2, 0x06),
    scheduled(3584, 0, 0xB2, 0x07),
    scheduled(4000, 0, 0x27, 0x84),
};
...
const scenarios = [_]Scenario{
    ...
    .{
        .name = "triad-mix",
        .description = "Three simultaneous FM channels with split pan and different pitches",
        .total_clocks = 2048,
        .writes = triad_mix_writes[0..],
    },
    .{
        .name = "dac-step",
    ...
    .{
        .name = "csm-rapid",
        .description = "CSM with Timer A period=0x3FF for extremely rapid retriggering",
        .total_clocks = 4096,
        .writes = csm_rapid_writes[0..],
    },
    .{
        .name = "csm-param-change",
        .description = "CSM with operator TL parameter changes mid-retriggering",
        .total_clocks = 1024,
        .writes = csm_param_change_writes[0..],
    },
    .{
        .name = "csm-all-algos",
        .description = "CSM cycling through all 8 FM algorithms during retriggering",
        .total_clocks = 4096,
        .writes = csm_all_algos_writes[0..],
    },
};
